...
def compute_with_skills_ab_mask(config, batch_size: int, val: bool = False) -> Optional[np.ndarray]:
    """Generate a boolean mask of shape (batch_size,) for A/B skill injection.

    mask[i] = True  → trajectory i receives skill prompt (skill arm)
    mask[i] = False → trajectory i gets no skill prompt  (no-skill arm)

    Rules (mirrors D2Skill):
    - Validation always returns None (all envs get skills).
    - Returns None when ab_rollout is disabled.
    - Within each rollout group of size rollout_n, the first
      ceil(rollout_n * ab_ratio) trajectories get skills.
    """
    if val:
        return None
    som_cfg = (config.env.get("skills_only_memory") or {})
    if not som_cfg.get("ab_rollout", False):
        return None
    if batch_size <= 0:
        return None
    rollout_n = int(config.env.rollout.get("n", 0) or 0) or 1
    ab_ratio = float(som_cfg.get("ab_ratio", 0.5))
    n_with = max(1, min(rollout_n, int(round(rollout_n * ab_ratio))))
...
class SearchEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Tuple[Dict[str, Any], List[Dict]]:
        ...
        # ── Replay-based verification: pin to specific question/ground_truth ──
        _pinned_task_kwargs = None
        if kwargs is not None:
            import numpy as _np_local
            kw0 = kwargs[0] if isinstance(kwargs, (list, _np_local.ndarray)) and len(kwargs) > 0 else kwargs
            if isinstance(kw0, dict) and 'question' in kw0:
                _pinned_task_kwargs = kw0  # caller passed a full task dict

        if _pinned_task_kwargs is not None:
            obs, infos = self.envs.reset(kwargs=[_pinned_task_kwargs])
        else:
            obs, infos = self.envs.reset(kwargs=kwargs)

        # Persist task kwargs for replay (expose question/ground_truth/data_source)
        if hasattr(self.envs, 'last_task_kwargs') and self.envs.last_task_kwargs:
            self.task_kwargs = list(self.envs.last_task_kwargs)
        else:
            self.task_kwargs = list(kwargs) if kwargs is not None else []
    ...
    def step(self, text_actions: List[str]):
        ...
        for i, info in enumerate(infos):
            ...
            info["projected_action"] = actions[i] if valids[i] else ""
...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs):
        # ── Replay-based verification: pin to a specific gamefile ──────────
        _pinned_gamefile = None
        if kwargs is not None:
            kw0 = kwargs[0] if isinstance(kwargs, (list, __import__('numpy').ndarray)) and len(kwargs) > 0 else kwargs
            if isinstance(kw0, dict):
                _pinned_gamefile = kw0.get('gamefile', None)

        if _pinned_gamefile:
            # Check if a list of gamefiles is provided (one per worker)
            _pinned_gamefiles = None
            if kwargs is not None:
                import numpy as _np_local
                kw_list = kwargs if isinstance(kwargs, (list, _np_local.ndarray)) else [kwargs]
                gfs = [kw.get('gamefile') for kw in kw_list
                       if isinstance(kw, dict) and kw.get('gamefile')]
                if len(gfs) > 1:
                    _pinned_gamefiles = gfs

            if _pinned_gamefiles and hasattr(self.envs, 'reset_all_with_gamefiles'):
                # Expand to full batch size: each task repeated group_n times
                group_n = getattr(self.envs, 'group_n', 1)
                gamefiles_expanded = []
                for gf in _pinned_gamefiles:
                    gamefiles_expanded.extend([gf] * group_n)
                # Pad to num_processes if needed
                while len(gamefiles_expanded) < self.envs.num_processes:
                    gamefiles_expanded.append(gamefiles_expanded[-1])
                gamefiles_expanded = gamefiles_expanded[:self.envs.num_processes]
                text_obs, image_obs, infos = self.envs.reset_all_with_gamefiles(gamefiles_expanded)
                print(f"[EnvReset] reset_all_with_gamefiles: {len(_pinned_gamefiles)} tasks "
                      f"× {group_n} workers = {len(gamefiles_expanded)} total")
            else:
                # Single gamefile: reset worker[0] and pad
                text_obs, image_obs, infos = self.envs.reset_with_gamefile(_pinned_gamefile)
                batch_size_full = self.envs.num_processes
                while len(text_obs) < batch_size_full:
                    text_obs.append(text_obs[0])
                    infos.append(infos[0])
                    if image_obs:
                        image_obs.append(image_obs[0])
        else:
        ...
        # A/B mask: True = skill arm, False = no-skill arm. None = all get skills.
        is_val = getattr(self, "val_rollout_always_skills", False)
        self.with_skills_mask = compute_with_skills_ab_mask(self.config, batch_size, val=is_val)
        ...
        if self.retrieval_memory is not None:
            ...
            # force_include_ids: guarantee updated skills appear in prompt.
            # force_skill_arm: override A/B mask so all trajectories use skills.
            force_ids = None
            force_skill_arm = False
            if kwargs is not None:
                kw0 = kwargs[0] if isinstance(kwargs, (list, np.ndarray)) and len(kwargs) > 0 else kwargs
                if isinstance(kw0, dict):
                    force_ids = kw0.get('force_include_ids')
                    force_skill_arm = bool(kw0.get('force_skill_arm', False))

            if force_skill_arm:
                self.with_skills_mask = np.ones(batch_size, dtype=bool)
                print(f"[EnvReset] force_skill_arm=True: all {batch_size} trajectories use skills")

            # Gamefile-based task_type detection is more reliable than keyword
            # matching on task descriptions.  Map AlfWorld gamefile substrings
            # to the canonical skill-bank category names.
            _GAMEFILE_TO_TYPE = {
            ...
            for i, task in enumerate(self.tasks):
                # No-skill arm: inject empty memories so prompt has no skill text
                if self.with_skills_mask is not None and not self.with_skills_mask[i]:
                    self.retrieved_memories.append(None)
                    continue

                # Derive task_type from gamefile (most reliable) falling back to
                # keyword detection inside retrieve() if gamefile is unavailable.
                task_type_hint = None
                gf = self.gamefile[i] if (self.gamefile and i < len(self.gamefile)) else None
                if gf:
                    for gf_key, tt in _GAMEFILE_TO_TYPE.items():
                        if gf_key in gf:
                            task_type_hint = tt
                            break
                ...
                self.retrieved_memories.append(memories)
                g_ids = [s.get('skill_id') for s in memories.get('general_skills', [])]
                t_ids = [s.get('skill_id') for s in memories.get('task_specific_skills', [])]
                print(f"[SkillRetrieval] traj={i} task_type={memories.get('task_type','')} "
                      f"gamefile_hint={task_type_hint} "
                      f"general({len(g_ids)})={g_ids} task_specific({len(t_ids)})={t_ids}")
    ...
    def build_text_obs(self, text_obs: List[str], admissible_actions: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            # Use retrieval memory template if enabled.
            # retrieved_memories[i] may be None for the no-skill arm of an A/B rollout.
            _mem_i = (self.retrieved_memories[i]
                      if (self.retrieved_memories is not None and i < len(self.retrieved_memories))
                      else None)
            has_skill = (self.retrieval_memory is not None and _mem_i is not None)
            use_retrieval = has_skill and not init

            if init and has_skill:
                # First step with skills: inject skill text without history
                memory_context = self.retrieval_memory.format_for_prompt(_mem_i)
                obs = ALFWORLD_TEMPLATE_WITH_MEMORY_INIT.format(
                    task_description=self.tasks[i],
                    retrieved_memories=memory_context,
...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        # ── Replay-based verification: pin to specific session_idx(s) ────────
        _pinned_session_idx = None
        if kwargs is not None:
            import numpy as _np_local
            kw0 = kwargs[0] if isinstance(kwargs, (list, _np_local.ndarray)) and len(kwargs) > 0 else kwargs
            if isinstance(kw0, dict):
                _pinned_session_idx = kw0.get('session_idx', None)

        if _pinned_session_idx is not None:
            # Check if multiple session_idxs are provided (one per task)
            _pinned_session_idxs = None
            if kwargs is not None:
                import numpy as _np_local
                kw_list = kwargs if isinstance(kwargs, (list, _np_local.ndarray)) else [kwargs]
                sis = [int(kw.get('session_idx')) for kw in kw_list
                       if isinstance(kw, dict) and kw.get('session_idx') is not None]
                if len(sis) > 1:
                    _pinned_session_idxs = sis

            if _pinned_session_idxs and hasattr(self.envs, 'reset_all_with_session_idxs'):
                # Expand: each task repeated group_n times
                group_n = getattr(self.envs, 'group_n', 1)
                session_idxs_expanded = []
                for si in _pinned_session_idxs:
                    session_idxs_expanded.extend([si] * group_n)
                while len(session_idxs_expanded) < self.envs.num_processes:
                    session_idxs_expanded.append(session_idxs_expanded[-1])
                session_idxs_expanded = session_idxs_expanded[:self.envs.num_processes]
                obs, infos = self.envs.reset_all_with_session_idxs(session_idxs_expanded)
                print(f"[EnvReset] reset_all_with_session_idxs: {len(_pinned_session_idxs)} tasks "
                      f"× {group_n} workers = {len(session_idxs_expanded)} total")
            else:
                # Single session_idx: reset worker[0] and pad
                obs, infos = self.envs.reset_with_session_idx(int(_pinned_session_idx))
                batch_size_full = self.envs.num_processes
                while len(obs) < batch_size_full:
                    obs.append(obs[0])
                    infos.append(infos[0])
        else:
            obs, infos = self.envs.reset()

        # Persist session idxs for replay
        if hasattr(self.envs, 'last_session_idxs'):
            self.session_idxs = list(self.envs.last_session_idxs)
        else:
            self.session_idxs = []
        ...
        # A/B mask: True = skill arm, False = no-skill arm
    ...
    def build_text_obs(self, text_obs: List[str], infos: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            elif use_retrieval and self.retrieved_memories[i] is not None:
                memory_context = self.retrieval_memory.format_for_prompt(
                    self.retrieved_memories[i]
                )
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        val_envs.val_rollout_always_skills = True  # val always uses full skills, no A/B split
        return envs, val_envs
