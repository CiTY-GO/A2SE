...
class WebshopMultiProcessEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.last_session_idxs = list(idx)  # expose for replay-based verification
    ...
    def reset_with_session_idx(self, session_idx: int):
        """Reset only worker[0] to a specific session index for replay-based verification."""
        obs, info = ray.get(self._workers[0].reset.remote(session_idx))
        self.last_session_idxs = [session_idx] * self.num_processes
        return [obs], [info]

    def reset_all_with_session_idxs(self, session_idxs: list):
        """Reset ALL workers in parallel, each to its own session_idx.

        Mirrors ALFWorld's reset_all_with_gamefiles().
        Args:
            session_idxs: list of ints, length == num_processes.
                          If shorter, last element is repeated.
        Returns same format as reset(): (obs_list, info_list)
        """
        futures = []
        for i in range(self.num_processes):
            si = int(session_idxs[i]) if i < len(session_idxs) else int(session_idxs[-1])
            futures.append(self._workers[i].reset.remote(si))
        ...
        self.last_session_idxs = [
            int(session_idxs[i]) if i < len(session_idxs) else int(session_idxs[-1])
            for i in range(self.num_processes)
        ]
        return obs_list, info_list
