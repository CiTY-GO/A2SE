...
def compute_grpo_counterfactual_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    traj_index: np.ndarray,
    with_skills_mask: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
):
    """
    Counterfactual Skill Advantage (CSA).
    Both skill arm and no-skill arm use the no-skill arm's mean/std as baseline.
    - No-skill arm: A = (R - mu_noskill) / sigma_noskill  (standard GRPO on no-skill only)
    - Skill  arm:  A = (R - mu_noskill) / sigma_noskill  (counterfactual: did skill help vs baseline?)
    When no-skill arm also fails (mu_noskill=0), skill arm advantage ~= 0 instead of negative.
    """
    scores = token_level_rewards.sum(dim=-1)

    noskill_id2score = defaultdict(list)
    seen_pairs = set()

    with torch.no_grad():
        bsz = scores.shape[0]

        # Collect no-skill arm episode scores per task group
        for i in range(bsz):
            if (index[i], traj_index[i]) in seen_pairs:
                continue
            seen_pairs.add((index[i], traj_index[i]))
            if not bool(with_skills_mask[i]):
                noskill_id2score[index[i]].append(scores[i])

        # Compute no-skill mean/std per group
        noskill_mean = {}
        noskill_std = {}
        for uid, ss in noskill_id2score.items():
            t = torch.stack(ss)
            noskill_mean[uid] = t.mean()
            noskill_std[uid] = t.std() if len(ss) > 1 else torch.tensor(epsilon)

        # Apply counterfactual advantage to all trajectories
        for i in range(bsz):
            uid = index[i]
            if uid not in noskill_mean:
                scores[i] = torch.tensor(0.0)
                continue
            mu = noskill_mean[uid]
            sigma = noskill_std[uid] + epsilon
            if norm_adv_by_std_in_grpo:
                scores[i] = (scores[i] - mu) / sigma
            else:
                scores[i] = scores[i] - mu
