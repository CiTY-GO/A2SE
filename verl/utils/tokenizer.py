...
# Monkey-patch huggingface_hub to allow absolute local paths as repo_id
try:
    import huggingface_hub.utils._validators as _hf_validators
    _orig_validate_repo_id = _hf_validators.validate_repo_id

    def _patched_validate_repo_id(repo_id, *args, **kwargs):
        if isinstance(repo_id, str) and os.path.isabs(os.path.expanduser(repo_id)):
            return  # 绝对路径或 ~ 路径直接跳过验证
        return _orig_validate_repo_id(repo_id, *args, **kwargs)
